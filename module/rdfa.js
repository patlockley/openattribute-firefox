...
Util.uri.refTo = function(base, uri){
    ...
    // fragment of base?
    if (uri.slice(i, i + 1) == '#' && base.length == i) 
        return uri.slice(i);
    while (i > 0 && uri[i - 1] != '/') 
        i--;
...
function makeTerm(val){
    ...
    if (typeof val == 'boolean') 
        return new RDFLiteral(val ? "1" : "0", undefined, RDFSymbol.prototype.XSDboolean);
...
function Namespace(nsuri){
    return function(ln){
        return new RDFSymbol(nsuri + (ln === undefined ? '' : ln))
    }
}
...
function RDFIndexedFormula(features){
    ...
    if (typeof features == 'undefined') 
        features = ["sameAs", "InverseFunctionalProperty", "FunctionalProperty"];
...
var RDFPlainFormula = function(){
    return RDFIndexedFormula([]);
} // No features
...
RDFA.each_attr_value = function(attr_values, attr_val_func){
    ...
    for (var i = 0; i < attr_value_arr.length; i++) {
		...
		//if (attr_values.indexOf("creativecommons") > -1) {
...
// PL ---> NY Added in 'nofollow'

XH.SPECIAL_RELS = ['next', 'prev', 'home', 'license', 'alternate', 'appendix', 'bookmark', 'cite', 'chapter', 'contents', 'copyright', 'glossary', 'help', 'icon', 'index', 'last', 'meta', 'p3pv1', 'role', 'section', 'subsection', 'start', 'stylesheet', 'up'];
	...
	//if (element!= undefined ) {
		...
		RDFA.each_attr_value(rel_value, function(rel_val){
			if (XH.SPECIAL_RELS.indexOf(rel_val) > -1) {
				...
				add_namespaces = true;
			}else if(rel_value=="nofollow"){
				
				href_value = RDFA.getNodeAttributeValue(element, 'href');
				
				if (href_value.indexOf("creativecommons") != -1) {
					new_rels.push(XH.XHTML_PREFIX + ':' + "license");
					add_namespaces = true;				
				}
...
RDFA.add_hanging_rel = function(hanging, subject, rel, base, namespaces){
    if (!hanging) 
        hanging = RDFA.init_hanging(subject, base, namespaces);
...
RDFA.traverse = function(element, subject, namespaces, lang, base, hanging){
	...
	if (attrs['href']){	
		...
		// PL ----> NY "This fixes the wikipedia bug that I have mentioned"
		
		// original line - explicit_object = attrs['href'];
